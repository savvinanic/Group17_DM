---
title: "Group17_IB9HP0 Data Management Report"
author: "5563324, 5530970, 2028065, 5562860, 5522715, 2024479"
date: "2024-03-18"
output:
  pdf_document:
    toc: yes
...
customer ($\underline{customer\_id}$, $\underline{\underline{promo\_code}}$, customer_firstname, customer_lastname, customer_title, customer_phone, customer_email, customer_membership, delivery_fee, customer_building, customer_street, customer_city, customer_postcode) 

2. Delivery 

delivery ($\underline{tracking\_number}$, $\underline{\underline{trans\_id}}$, shipment_method, tracking_status, estimated_delivery_date, estimated_delivery_time, actual_delivery_date, actual_delivery_time, delivery_instructions)

3. Supplier 

supplier ($\underline{supplier\_id}$, supplier_name, supplier_phone, supplier_email, supplier_building, supplier_street, supplier_city, supplier_postcode) 

4. Transaction 

transaction ($\underline{trans\_id}$, $\underline{\underline{order\_id}}$, trans_date, trans_time) 

5. Product category 

product_category ($\underline{category\_name}$, parent_category_id) 

6. Product 

product ($\underline{product\_id}$, $\underline{\underline{supplier\_id}}$, $\underline{\underline{category\_name}}$, product_name, product_weight, product_length, product_height, product_width, product_price) 

7. Promotion 

promotion ($\underline{promo\_code}$, promo_start_date, promo_expire_date, percentage_discount) 

Finally, due to the many-to-many (M:N) relationship between the customer and product tables, it is necessary to create an additional, separate table named 'order', which will contain additional attributes. The three attributes “order_id”, “customer_id” and “product_id” form a composite primary key for the “order” table, while “customer_id” and “product_id” also serve as a foreign key from the customer table and product table respectively. 

8. Order 

order ($\underline{order\_id}$ , $\underline{customer\_id}$, $\underline{product\_id}$, product_qty, order_date, order_time)
```{r echo=FALSE}
...
Our group chose to generate the data before creating the empty tables in SQL. This approach facilitated the verification of whether all tables were normalized up to the third normal form (3NF), ensuring optimal data organization and integrity.  

Therefore, before importing the synthetic dataset, we checked if our tables were in the First Normal Form (1NF), which required atomic values. For instance, the “customer_address” attribute in the customer table required us to separate columns for “customer_building”, “customer_street”, and “customer_city”, ensuring each contained only a singular value 

In achieving 2NF and 3NF, we divided the customer table into two: customer_basic_info for personal details, and customer_membership for membership and fees, correcting a transitive dependency where 'delivery_fee' relied on 'customer_membership', not the 'customer_id' primary key. The logical schema is shown below.

a) Customer Basic Information

customer_baisc_info ($\underline{customer\_id}$, $\underline{\underline{promo\_code}}$, customer_firstname, customer_lastname, customer_title, customer_phone, customer_email, customer_building, customer_street, customer_city, customer_postcode) 

b) Customer Membership

customer_membership ($\underline{customer\_id}$, $\underline{customer\_membership}$, delivery_fee) 



For order table where the composite key of “order_id”, “customer_id”, and “product_id” is the composite primary key, we normalized it into 2NF and then it satisfy 3NF at the same time. Since we have {order_id, customer_id, product_id} $\rightarrow$ {order_date, order_time}, but also {order_id, customer_id} $\rightarrow$ {order_date, order_time}, the 1NF is not in 2NF, meaning "order_date" and "order_time" are partially dependent on the composite primary. Therefore, we separate order table into order_datetime and order_products_info. The logical schema for 3NF is shown below.

a) Order Products Information

order_products_info ($\underline{order\_id}$ , $\underline{customer\_id}$, $\underline{product\_id}$, product_qty)

b) Order Datetime

order_datetime ($\underline{order\_id}$ , $\underline{customer\_id}$, order_date, order_time) 
...
delivery_tracking ($\underline{tracking\_number}$, $\underline{\underline{trans\_id}}$, delivery_instructions)

b) Estimated Delivery Date

estimated_delivery_date ($\underline{tracking\_number}$, $\underline{shipment\_method}$, estimated_delivery_date, estimated_delivery_time)

c) Actual Delivery Date

actual_delivery_date ($\underline{tracking\_number}$, $\underline{tracking\_status}$, actual_delivery_date, actual_delivery_time)
...
```{sql connection=connection}
SELECT o.order_id, prm.percentage_discount, m.delivery_fee, 
SUM(p.product_price*o.product_qty) AS order_price, 
ROUND(
  SUM(p.product_price* o.product_qty)*
  (1 - CAST(prm.percentage_discount AS REAL) / 100) + m.delivery_fee,2
  ) AS trans_amount
FROM "order_products_info" o, "product" p, "promotion" prm, "customer_basic_info" c, "customer_membership" m
WHERE o.customer_id = m.customer_id AND o.product_id = p.product_id 
AND c.promo_code = prm.promo_code AND m.customer_id = c.customer_id
GROUP BY 
    o.order_id;
...
## Part 4.1 - 4.2: Data Visualisation
...
SELECT op.order_id, op.customer_id, op.product_id, 
p.parent_category_id, op.product_qty, p.product_price, od.order_date
...
SELECT o.customer_id, c.customer_city, o.order_id, d.order_date,
ROUND(
SUM(p.product_price* o.product_qty)*(1 - CAST(prm.percentage_discount AS REAL) / 100),2) AS revenue
